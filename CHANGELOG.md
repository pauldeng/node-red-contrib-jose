...
- `jose-key` configuration node: one key per family (signing or encryption) with one bound algorithm and `auto` selection from the material. Sources: shared secret (strict canonical decoding, RFC 7518 minimum lengths), PEM private key (PKCS#8, PKCS#1, SEC1, encrypted with a passphrase), PEM public key or X.509 certificate, a single JSON Web Key (its `alg`, `use` and `key_ops` are enforced) a pasted JSON Web Key Set with an explicit algorithm list for verification, and a remote JSON Web Key Set URL (https only, loopback http opt-in for testing, cache and timeout settings, jose's kid selection and cooldown; with several allowed algorithms only keys that declare their `alg` are eligible). RSA, P-256/P-384/P-521, Ed25519 and X25519 keys; `RS*`, `PS*`, `ES*`, `Ed25519`, `RSA-OAEP-256` and `ECDH-ES+A256KW`. All key material lives in password-type credentials; `${ENV}` references are honoured.
