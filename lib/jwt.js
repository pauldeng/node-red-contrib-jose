...
function consumeOptions(config) {
  ...
  const audience = config.audience === undefined ? "" : config.audience;
  if (typeof audience !== "string") throw coded("INVALID_INPUT", "audience must be a comma-separated string");
  if (audience.trim() !== "") {
    const values = audience
      .split(",")
      .map((value) => value.trim())
      .filter(Boolean);
    if (!values.length) throw coded("INVALID_INPUT", "audience must contain a value");
    options.audience = values.length === 1 ? values[0] : values;
  }
  return options;
}
